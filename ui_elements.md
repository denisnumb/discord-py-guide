...
Теперь давайте создадим выпадающи список. Из документации по классу [`discord.ui.Select`][14] видим, что можем передать в конструктор класса парамтры:
...
Далее реализуем обработчик выбора параметра. Сделаем так, чтобы при выборе элемента бот изменял текст сообщения на "<Пользователь> выбрал <выбор>"
