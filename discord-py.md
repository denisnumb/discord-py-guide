...
*Так или иначе, для работы со slash-командами будет не лишним изучить материал по prefix-командам.*
...
```
 ...
 В результате должно получиться что-то [**такое**][33]. Данный бот умеет обрабатывать и события и отдельные *prefix*-команды.
...
В декоратор можно сразу передать параметры **названия** и **описания** команды:
```py
...
Далее создаем функцию команды. Поскольку дискорд обрабатывает команды интерактивно (![image](https://user-images.githubusercontent.com/61795655/206803051-2c32a095-b8df-4fa1-ad93-dd1ef8c12a7e.png)), необходимо что-то делать с контекстом выполнения команды:
